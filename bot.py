import discord
from discord.ext import commands
import os
import asyncio
from keep_alive import keep_alive

class ChiringuitoBot(commands.Bot):
    def __init__(self):
        intents = discord.Intents.default()
        intents.message_content = True  # Requisito indispensable en el Developer Portal
        super().__init__(command_prefix='!', intents=intents, help_command=None)
        
        # Constantes fijas exclusivas de LaLiga EA Sports
        self.BASE_URL = "https://api-sports.io"
        self.LALIGA_ID = 140
        self.CURRENT_SEASON = 2024
        self.HEADERS = {'x-apisports-key': os.getenv('FOOTBALL_API_KEY')}

    async def setup_hook(self):
        """Carga de forma asíncrona los comandos antes de conectar el cliente."""
        for filename in os.listdir('./cogs'):
            if filename.endswith('.py'):
                # Importación limpia del módulo quitando la extensión .py
                await self.load_extension(f'cogs.{filename[:-3]}')
                print(f'Comando modular cargado: {filename}')

bot = ChiringuitoBot()

@bot.event
async def on_ready():
    print(f'Conectado exitosamente como {bot.user.name} | Exclusivo LaLiga EA Sports')

@bot.command(name='ayuda')
async def ayuda(ctx):
    embed = discord.Embed(title="📋 Comandos de LaLiga EA Sports", color=discord.Color.gold())
    embed.add_field(name="`!tabla`", value="Muestra la clasificación de Primera División.", inline=False)
    embed.add_field(name="`!jornada`", value="Muestra los partidos de la jornada actual.", inline=False)
    embed.add_field(name="`!resultados`", value="Muestra los últimos 5 marcadores finalizados.", inline=False)
    await ctx.send(embed=embed)

async def main():
    # Inicializar el servidor web de Render para evitar suspensiones de servicio
    keep_alive()
    async with bot:
        await bot.start(os.getenv('DISCORD_TOKEN'))

# Ejecución controlada dentro del bucle de eventos asíncronos de Python
if __name__ == "__main__":
    asyncio.run(main())
